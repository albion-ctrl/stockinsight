...
const supabaseKey = process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY || 'sb_publishable_nH7pEleg321SkO1hHO9B9A_sbPtj12p';
...
function buildMarketListings(v: typeof REAL_VEHICLES[number]) {
  const base = v.price;
  const marketDelta = Math.random() * 0.1 - 0.05;
  return [
    { source: "AutoScout24", url: "https://www.autoscout24.nl", title: v.name, price: Math.round(base * (0.90 + marketDelta)), year: v.year, mileage: v.mileage + 15000, location: NL_LOCATIONS[1], days_online: Math.floor(Math.random() * 30) + 1, dealer: DEALERS[0] },
    { source: "Gaspedaal", url: "https://www.gaspedaal.nl", title: v.name, price: Math.round(base * (0.91 + marketDelta * 0.8)), year: v.year - 1, mileage: v.mileage + 28000, location: NL_LOCATIONS[3], days_online: Math.floor(Math.random() * 40) + 1, dealer: DEALERS[2] },
    { source: "Marktplaats", url: "https://www.marktplaats.nl", title: v.name, price: Math.round(base * (0.89 + marketDelta * 0.9)), year: v.year, mileage: v.mileage + 8000, location: NL_LOCATIONS[5], days_online: Math.floor(Math.random() * 20) + 1, dealer: DEALERS[4] },
  ];
}

async function seedDatabase() {
  console.log("Seeding database with real Den Engelsen vehicles...");
  ...
  for (const v of REAL_VEHICLES) {
    ...
    const marketListings = buildMarketListings(v);
    ...
    if (vehicleError) {
      console.error(`Error inserting vehicle ${v.id}:`, vehicleError);
    }

    // Insert action items
    for (const action of actions) {
      const { error: actionError } = await supabase.from('action_items').upsert(action, { onConflict: 'id' });
      if (actionError) console.error(`Error inserting action ${action.id}:`, actionError);
    }

    // Insert market listings
    for (const listing of marketListings) {
      const { error: listingError } = await supabase.from('market_listings').insert({
        vehicle_id: v.id,
        ...listing,
      });
      if (listingError) console.error(`Error inserting listing:`, listingError);
    }

    console.log(`Inserted: ${v.name} (${v.branch})`);
  }
