...
function calcInterestCost(price: number, days: number): number {
  return Math.round((price * 0.055 * days) / 365);
}
...
function getUrgencyScore(days: number, price: number): number {
  const dailyInterest = (price * 0.055) / 365;
...
export async function GET() {
  try {
    // First, try to create tables using SQL
    console.log("Creating tables...");

    // Create tables via direct SQL
    ...
    // If that doesn't work, just try to insert and let it fail if tables don't exist
    // The user will need to create tables manually in Supabase dashboard
    ...
    for (const v of VEHICLES) {
      ...
      const marketListings = [
        { source: "AutoScout24", url: "https://www.autoscout24.nl", price: Math.round(v.price * (0.90 + marketDelta)), location: "Rotterdam", days_online: Math.floor(Math.random() * 30) + 1, dealer: "Van der Berg Trucks" },
        { source: "Gaspedaal", url: "https://www.gaspedaal.nl", price: Math.round(v.price * (0.91 + marketDelta * 0.8)), location: "The Hague", days_online: Math.floor(Math.random() * 40) + 1, dealer: "De Vries Used Cars" },
        { source: "Marktplaats", url: "https://www.marktplaats.nl", price: Math.round(v.price * (0.89 + marketDelta * 0.9)), location: "Tilburg", days_online: Math.floor(Math.random() * 20) + 1, dealer: "Hofman Vehicles" },
      ];
      ...
      const { error } = await supabase.from('vehicles').insert([{
        ...
        interest_cost: calcInterestCost(v.price, days),
        urgency_score: getUrgencyScore(days, v.price),
        recommended_price: calcRecommendedPrice(v.price, days),
        discount_pct: getDiscountPct(days),
        market_delta_pct: marketDeltaPct,
        pending_actions: actions.filter(a => !a.completed).length,
      }]);

      if (error) {
        results.push({ id: v.id, status: "error", error: error.message });
        continue;
      }
      ...
      results.push({ id: v.id, status: "success" });
    }
    ...
    return NextResponse.json({ 
      success: true, 
      message: `Successfully inserted ${successCount} of ${VEHICLES.length} vehicles`,
      results 
    });
    ...
    return NextResponse.json({ 
      success: false, 
      error: error.message,
      instructions: "Please create the tables in Supabase dashboard first using the SQL in supabase/schema.sql"
    }, { status: 500 });
  }
}